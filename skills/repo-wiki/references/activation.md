...
1. **Bootstrap installs the triggers.** `kb.py init` or `kb.py plumbing` (both
   plumbing-only, order-independent) writes four hooks into `.claude/settings.json`
   (SessionStart, UserPromptSubmit, PreCompact, SessionEnd), installs
   `.git/hooks/post-commit`, and gitignores the local ingest watermark. Neither
   creates wiki structure — structure is always agreed first and then scaffolded via
   `kb scaffold --recommended|--only|--add`. Plumbing is fully independent of wiki
   structure — run it before or after `kb scaffold`. One-time setup, not recurring
   willpower.
