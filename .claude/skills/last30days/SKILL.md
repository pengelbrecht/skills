...
$ARGUMENTS
LAST30DAYS_TOPIC
# Resolve the script across known install roots (~/.claude or ~/.agents)
LAST30DAYS_SCRIPT="$(find ~/.claude/skills ~/.agents/skills -path '*last30days/scripts/last30days.py' 2>/dev/null | head -1)"
python3 "$LAST30DAYS_SCRIPT" "$(cat "$TOPIC_FILE")" --emit=compact 2>&1
```
